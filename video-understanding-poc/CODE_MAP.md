...
| 修改人脸识别分支（Phase 4 · Step 20/26；已接入事件管线 `--face`，含人脸库比对） | `app/face.py`（InsightFace 检测+对齐+512维 embedding+质量评估+最佳脸/多帧融合+对到 track_id）；接线 `app/event_pipeline.py` `_attach_faces`（提脸→**人脸 gallery 查/建档**：清晰脸入库高置信命中、糊脸只查不建防污染→写 `face.matched/face_subject_id`）；阈值 `FACE_HIT_THRESH/FACE_NEW_THRESH`（ArcFace 余弦，独立于 ReID）；配置 `FACE_*` |
| 修改三路身份融合（Phase 4 · A 汇聚，人脸+人形+步态→统一身份） | `app/services/identity_fusion.py`（`fuse_identity`：按质量加权融合三路—清晰脸主导/糊脸降权退人形步态/多路一致加成→`confidence/resolved/agreed/primary/sources`）；接线 `app/event_pipeline.py`（三路认完后逐 track 融合，写 `ident['fused']`，注入 identity_context grounding）；权重 `IDENTITY_W_*`/`IDENTITY_RESOLVE_THRESH` |
| 修改跨 track 三路合并（Phase 4 · A 汇聚，跨轨迹判同一人） | `app/event_pipeline.py` `_merge_tracks_cross_route`（并查集：人脸库/人形库/步态库**任一路**库编号相同的 track 并成一个 subject；清晰命中脸/命中步态才当锚点，糊脸不锚；多路印证写 `merge_routes/merge_agree`，改写后 `subject_id` 统一→下游 `_group_people` 自然归并；本会话新增，补 `_stitch_orphans` 只用人形一路的短板） |
...
| 跑/改 身份感知·多帧事件理解 **端到端**（Phase 4 · Step 24，把上面叶子串成一条流） | `app/event_pipeline.py`（`analyze_event_stream`：抽帧①→YOLO+ByteTrack→语义事件标注→ReID认人+灰区轨迹缝合(同视频内并同一人)+跨track三路合并(`_merge_tracks_cross_route`)→流式分窗(活动段+时长上限)→选帧②→身份打包→`understand_event`）+ CLI `scripts/event_understand_demo.py`（`--dry-run` 不调LLM验链路；`--max-window-seconds` 调窗长；`--stitch-thresh` 调缝合阈值(默认 EVENT_STITCH_THRESH=0.45)；默认视频 `data/samples/mixkit_31372.mp4`） |
