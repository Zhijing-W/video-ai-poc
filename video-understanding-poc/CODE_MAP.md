...
| 修改步态识别分支（Phase 4 · Step 27；第三路身份信号） | `app/gait.py`（**SkeletonGait++**/OpenGait，GREW 权重，CPU 建网+载权重+复用其热图预处理+自构张量喂 forward → 4096维步态向量；剪影用 yolov8-seg，姿态用 yolov8-pose；`extract_persons`/`embed_track`）；配置 `GAIT_*`（OpenGait 路径/权重在 git 仓库外）。无脸/背身/远景兜底，上云改 `GAIT_DEVICE=cuda` |
