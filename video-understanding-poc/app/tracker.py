"""ByteTrack 多目标跟踪层（Phase 3 · Step 11）。
...
实现：直接复用 **ultralytics 内置的 ByteTrack**（`ultralytics.trackers.byte_tracker.BYTETracker`）——
成熟、无需额外模型、仅依赖 `lap`（线性分配）。检测仍走 `detector._predict`（共享单例权重，
不重复推理），本层只负责"把这帧的框关联到已有轨迹/新建轨迹"。
...
高分框先做一段匹配，未匹配的轨迹再用低分框做二段匹配（不轻易丢目标），故抗短遮挡。
"""
...
# session_id -> {"tracker": BYTETracker, "lock": Lock}
_trackers: dict[str, dict] = {}
_registry_lock = threading.Lock()


def _build_args() -> types.SimpleNamespace:
    """把 settings 里的 ByteTrack 阈值打包成 BYTETracker 需要的 args 命名空间。"""
...
def _get_entry(session_id: str) -> dict:
    """懒加载：按 session 取（或新建）一个 BYTETracker 实例及其专属锁。"""
    with _registry_lock:
        entry = _trackers.get(session_id)
        if entry is None:
            from ultralytics.trackers.byte_tracker import BYTETracker

            entry = {"tracker": BYTETracker(_build_args()), "lock": threading.Lock()}
