...
三路的主体关系由 identity.resolution 负责解析；本模块只按质量加权计算身份置信度：
  - **质量自适应**：清晰正脸权重最高、糊脸降权退人形/步态（攻"人脸模糊"的最后一环）；
  - **多路线参与加成**：两路以上提供有效信号时增加置信度；
  - **互补兜底**：背身/远处看不到脸时，靠人形/步态撑住身份。

纯聚合、无副作用：产出 {confidence, sources, multi_source, resolved}；agreed 仅为兼容旧字段。
"""
...
def score_identity_confidence(ident: dict) -> dict:
...
def fuse_multimodal_identity(ident: dict) -> dict:
    return score_identity_confidence(ident)


__all__ = ["score_identity_confidence", "fuse_multimodal_identity"]
