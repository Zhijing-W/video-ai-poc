...
> 🚀 **性能优先原则**：本项目**最终上云（GPU）**，因此选型**以精度/效果优先，不为本地运行流畅而选小模型**。本地只是开发期验证；模型该重就重（如 AdaFace、3DMM、buffalo_l 全模块），云上跑。
...
### 3.3 多帧窗口选择（选帧②）—— **事件驱动**，不是像素/ffmpeg 驱动

> ⚠️ **关键原则**：**"图像变化大" ≠ "发生了事件"**。像素变化可能只是光照闪/风吹/抖动/曝光变化。所以**事件由语义信号定义**（检测/跟踪/身份/provider 的输出），**不是** ffmpeg 的 scene-change / 像素差。

**ffmpeg 的角色**：只当"取帧搬运工"（解码 + 选帧①定时均匀抽帧），**不当"判断事件的大脑"**。之前的 `extract_frames_smart`（ffmpeg scene-change 选帧）按此原则**不用于选关键帧**；选帧①退回最朴素的定时均匀抽帧即可。

**事件的语义定义**（来自上游，非像素）：新轨迹出生（有人/物进入）、轨迹消失（离开）、身份事件（库命中 / 开放集登记新主体）、provider 事件（包裹/跌倒/越界…）、计数变化。

**选帧②（`app/keyframe.py`）规则**——它**接收每帧的语义事件标注**（由检测/跟踪/身份算好），不自己算场景变化：
1. **有事件的帧 → 必留**（进/离开、身份命中、provider 报警）；
2. **每条 track 的最佳帧 → 留一张**（最清晰，给认人/叙述）；
3. **其余"无事件、太像"的帧 → 去重丢掉**（这里才用相邻相似度，**只为去冗余，不定义事件**）；
4. **保时序 + 留事件前后各一帧上下文**（LLM 才看得懂"发生了什么"）。

把"调用粒度"从 Phase 2 的"逐帧门控"转向"**每事件一次事件理解**"，与客户一致。**事件的定义权在检测/跟踪层，不在像素层、更不在 ffmpeg。**
...
### 3.8 攻"人脸模糊"的进阶武器（P1 · 性能优先，上云用重模型）

> 调研结论（2025）：对**模糊/低清人脸**，按 ROI 从高到低排序，不是单靠某一招，而是**多 cue 融合**。下面四件挂进我们已有的多线索融合（`track_fusion`），用 bad-case 评估逐个量化"加它救回多少"。

| # | 武器 | 作用 | 现状 / 选型 |
|---|---|---|---|
| ① | **ArcFace → AdaFace / MagFace** | 专为低质量/低清人脸训练的识别模型，**单项提升最大** | 现为 ArcFace(buffalo_l)；`face.py` 已可插拔后端，可换 AdaFace |
| ② | **多帧模板聚合** | 视频场景 SOTA：几帧糊→质量加权聚合成强模板 | ✅ 已实现（`face.fuse_embeddings` + `track_fusion`） |
| ③ | **3D 几何 cue（用户想法）** | 用**3D 形状/身份系数**（颧骨/鼻梁/下巴等几何）认人，对姿态+中度模糊鲁棒；纹理糊但几何还在 | 🔵 P1 新增。轻量起步：**打开 buffalo_l 已含的 3D-68 landmark（`1k3d68.onnx`，之前为提速关掉）**；重型升级：**MICA / Deep3DFaceRecon** 的 3DMM 身份系数（上云 GPU 跑） |
| ④ | **人脸超分**（GFP-GAN / CodeFormer） | 识别前把糊脸拉清作预处理 | 🔵 P1 可选；收益递减，排最后 |

> ⚠️ **诚实边界**（调研）：3D **不是银弹**——从**极小/极糊**脸重建 3D 本身不可靠（landmark 都定不准），它主要救**姿态 + 中度模糊**；对**超小脸(<32px)**，经验上 **AdaFace 跨分辨率 + 多帧聚合** 更有效。故 **3D 当互补 cue 融合，不单独用**。
> 🧪 **怎么证明**：把 ①②③④ 做成融合里的可开关 cue，用 `eval_phase3.py` 在模糊人脸数据上跑"逐个叠加"的对比表——这正是说服客户的弹药。
...
Step 27b 人脸进阶武器（§3.8）：ArcFace→AdaFace 升级 / 打开 3D-68 landmark 几何 cue / 多帧聚合 / 超分  ⬜ P1
Step 28  bad-case 评估扩展：模糊人脸数据上 baseline vs +增强/+多帧脸/+人形/+gait/+3D 对比表（弹药）   ⬜ P1
