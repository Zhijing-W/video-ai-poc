...
> **文档性质**：实习论文阶段性初稿  
> **正式实验**：协议A（人脸、人形、步态消融）、协议B3（独立训练身份校准的人脸模型对比）与固定Check-in Gallery多后端超分身份保持实验<br>
...
4.5%和5.1%，陌生人实际误识率分别为1.5%和2.8%。B3中的GFPGAN仅按旧模糊触发规则增强
3个独立帧，样本不足，因此该结果只保留为触发诊断。

为直接评价超分是否保留身份，本文另以MEVID actor check-in正面照建立固定ArcFace Gallery，
冻结316条官方Query的face-best，并在40条预先定义的recoverable Query上比较原图、GFPGAN、
CodeFormer与Real-ESRGAN。原图Rank-1为65.0%，三种超分分别为20.0%、27.5%和45.0%；
两种纯缩放控制均保持65.0%。GFPGAN和CodeFormer没有救回原图错误样本，却分别损害18条和
15条原本正确样本；Real-ESRGAN救回1条、损害9条。当前证据不支持用超分输出替换ArcFace原始
对齐脸。
...
**关键词**：监控身份识别；低质量人脸；行人重识别；步态识别；人脸超分；身份保持；MEVID
...
- GFPGAN、CodeFormer和Real-ESRGAN是否真的改善识别，而不仅仅是让图片看起来更清晰？
...
GFPGAN利用生成式人脸先验修复低质量人脸[6]。CodeFormer通过离散码本与可调保真权重重建人脸，
本文固定使用保真优先的`w=1`设置[7]。Real-ESRGAN是通用真实图像超分方法，不使用专用人脸
生成先验，本文使用x2plus后端[8]。三种方法都可能改善视觉效果，但只有身份识别配对实验才能判断
输出是否仍保留原身份。
...
## 6. 协议B3：公平比较ArcFace、AdaFace与GFPGAN触发诊断
...
（3.5%与2.8%）。
该差异样本很少，不能据此判断GFPGAN稳定提高了陌生人相似度。因此B3只能得出：

> 按B3旧模糊规则触发的GFPGAN样本太少，尚不足以评价总体增益。

该结论不再承担本文对超分身份保持的最终回答。第7节使用固定Check-in Gallery和预先冻结的
40条recoverable Query，对GFPGAN、CodeFormer与Real-ESRGAN进行全量配对评测。
...
## 7. 固定Check-in Gallery的多后端超分身份保持实验

### 7.1 冻结协议

B3只触发3个独立帧，无法回答超分对远场小脸身份保持的总体影响。为避免按识别结果挑样本，本文
另建固定协议：以actor check-in正面照作为唯一Gallery；对316条官方Query确定性抽取24帧，
使用产品共享的人脸质量排序冻结唯一face-best；随后固定direct、recoverable、unusable和none
可用状态。最终156条Query具有可对齐脸，其中40条在看结果前已按20--28像素尺寸规则归入
recoverable主组。

所有方案共享同一Gallery、Query face-best、ArcFace和接受阈值。A直接使用原始对齐脸；
B1--B3对156张可对齐脸分别运行GFPGAN、CodeFormer `w=1`和Real-ESRGAN x2plus；C1--C3
仅在冻结recoverable组采用对应后端，direct继续使用A，unusable和none拒绝。三种后端输出均
归一化为112×112后再提取ArcFace特征。另设两种只放大再压回112×112的控制组，用于排除尺寸
往返本身的影响。

### 7.2 Recoverable主结果

![冻结recoverable组Rank-1与PID聚类置信区间](../paper/figures/recoverable_rank1_ci.png)

| 处理 | Rank-1 | Rank-5 | MRR | 相对原图变化 |
|---|---:|---:|---:|---:|
| A 原图 | **26/40（65.0%）** | 28/40（70.0%） | 0.693 | — |
| C1 GFPGAN | 8/40（20.0%） | 12/40（30.0%） | 0.259 | -45.0个百分点 |
| C2 CodeFormer `w=1` | 11/40（27.5%） | 16/40（40.0%） | 0.346 | -37.5个百分点 |
| C3 Real-ESRGAN x2plus | 18/40（45.0%） | 26/40（65.0%） | 0.532 | -20.0个百分点 |
| Resize512控制 | 26/40（65.0%） | 29/40（72.5%） | 0.690 | 0.0个百分点 |
| Resize-x2控制 | 26/40（65.0%） | 29/40（72.5%） | 0.688 | 0.0个百分点 |

三种超分均低于原图。两种缩放控制的Rank-1与A完全相同，说明损失不是112×112输入经历尺寸
往返造成，而是超分模型改变了身份相关特征。Real-ESRGAN在三种后端中最接近原图，但仍低
20个百分点。

### 7.3 救回与损害的配对分析

![三种后端相对原图的救回率与损害率](../paper/figures/rescue_harm_transitions.png)

| 方法 | 错→对 | 对→错 | Rank-1净变化 | PID聚类95%置信区间 | PID置换检验p值 |
|---|---:|---:|---:|---:|---:|
| GFPGAN | 0 | 18 | -45.0个百分点 | [-61.5, -33.3]个百分点 | 0.00125 |
| CodeFormer | 0 | 15 | -37.5个百分点 | [-53.6, -25.0]个百分点 | 0.00410 |
| Real-ESRGAN | 1 | 9 | -20.0个百分点 | [-33.3, +3.6]个百分点 | 0.31213 |

GFPGAN和CodeFormer没有救回任何原图错误样本，却损害了大量原本正确样本；PID聚类推断仍显示
显著负向变化。Real-ESRGAN出现1条救回，但损害9条，且40条样本不足以证明其人群级差异稳定。

### 7.4 身份漂移、FIQA与运行开销

| 后端 | 成功/失败 | 平均秒/脸 | 原图与超分特征余弦相似度 | FIQA平均变化 |
|---|---:|---:|---:|---:|
| GFPGAN | 156/0 | 0.236 | 0.402 | +0.468 |
| CodeFormer `w=1` | 156/0 | 0.324 | 0.584 | +0.154 |
| Real-ESRGAN x2plus | 156/0 | **0.075** | **0.783** | -0.142 |

三种后端均完成156张可对齐脸的处理。GFPGAN使FIQA上升最多，但与原图ArcFace特征最不相似，
且身份识别结果最差；Real-ESRGAN的FIQA下降，却保留了最多身份信息。因此FIQA只能作为单图质量
诊断，不能作为超分保留身份的证明，也不能据此决定是否接受恢复结果。

### 7.5 视觉审计与产品结论

![冻结质量样本的三后端视觉对比](../paper/figures/qualitative_grid.png)
...
| 超分是否改善身份识别？ | 否；冻结recoverable组中原图Rank-1为65.0%，GFPGAN、CodeFormer和Real-ESRGAN分别为20.0%、27.5%和45.0%。 |
...
6. 身份识别默认使用原始对齐脸，超分输出只作为人工画面增强或离线诊断；
7. 模型对比、身份状态机验证和完整产品实验必须分层进行。
...
5. 协议B3的5次测试划分并非完全独立样本，置信区间主要反映身份组合变化；
6. 多后端超分实验的recoverable主组只有40条Query，Real-ESRGAN仍需更多独立身份复验；
7. 超分结论仅适用于当前模型版本、ArcFace与MEVID监控条件，不能推广到所有画面增强任务。
...
[7] S. Zhou, K. C. K. Chan, C. Li, and C. C. Loy. “Towards Robust Blind Face Restoration with Codebook Lookup Transformer.” *NeurIPS*, 2022.<br>
https://arxiv.org/abs/2206.11253

[8] X. Wang, L. Xie, C. Dong, and Y. Shan. “Real-ESRGAN: Training Real-World Blind Super-Resolution with Pure Synthetic Data.” *ICCV Workshops*, 2021.<br>
...
阈值校准、GFPGAN触发率和质量原因混合等问题。ArcFace与AdaFace的正式结论以训练身份和测试
身份完全独立的协议B3为准；超分身份保持的正式结论以第7节固定Check-in Gallery实验为准。
