...
from app.event_analysis_pipeline import analyze_event_stream  # noqa: E402
...
def main() -> int:
    ...
    for win in payload["windows"]:
        ...
        if win.get("scene_context"):
            print("  --- 注入 LLM 的场景文字（OCR，不代表身份）---")
            print("  " + win["scene_context"].strip().replace("\n", "\n  "))
        if win.get("object_context"):
            print("  --- 注入 LLM 的物体上下文（YOLO，非身份）---")
            print("  " + win["object_context"].strip().replace("\n", "\n  "))
        if "event" in win:
            ev = win["event"]
            print("  --- 事件理解（gpt-4o）---")
